...
summary: 从一份脚本和 27 个本地素材出发，经过 17 轮迭代，在千问办公中完成一个可在线预览的 3D 粒子球体 Showreel 动画网站，最终推送到 GitHub 并启用 Pages。
author: JZCreative
date: "2026-07-15"
category: 创意开发
difficulty: 进阶
...
但手写这个动画的 3D 数学、粒子分布、时间线编排和 Mockup 布局非常耗时。所以我把它交给千问办公，用自然语言描述需求，让它逐步生成并迭代。
...
本次任务没有安装额外的 Skill，全程使用千问办公的内置能力：

| 能力 | 用途 | 来源或安装方式 |
| --- | --- | --- |
| 本地文件读写 | 读取素材文件、写入生成的 HTML | 千问办公内置 |
| Bash 执行 | 启动 HTTP 服务器、检查视频尺寸、Git 操作 | 千问办公内置 |
| 网页预览 | 在浏览器中实时预览动画效果 | 千问办公内置 |
...
千问办公读取了素材目录，生成了第一版 HTML，并启动了本地 HTTP 服务器让我预览。
...
1. **先描述效果，再描述实现**：告诉千问办公你想要什么视觉效果，而不是直接指定技术方案。它会在尝试中找到最优解（比如从 CSS 3D 到软件投影的转变）。
